...
public class UserController {
    ...
    public String postRegister(@Validated User user,
    ...
    public String getUser(@PathVariable(name = "id", required = false)
    ...
    public String postUser(@Validated User user, BindingResult res, Model model) {
        if(res.hasErrors()) {
            // エラーあり
            return getUser(null, model); //id欄をあえてnullにして条件分岐
        }
        // User登録
        service.saveUser(user);
        // 一覧画面にリダイレクト
    ...
    public String deleteRun(@RequestParam(name="idck")
                            Set<Integer> idck, Model model) {
        // Userを一括削除
